...
class CartService
{
    ...
    public function addItem(int $itemId, int $quantity = 1, array $options = []): CartItem {
        $cart = $this->getCart();
        $item = $this->findPurchasableItem($itemId);
    ...
    public function mergeGuestCart(?string $sessionId = null): ?string{
        ...
        return DB::transaction(function () use ($guestCart, $userCart) {
            $discountCode = $userCart->discount_code
                ?: $guestCart->discount_code;
    ...
    private function refreshMergedDiscount(Cart $cart, ?string $discountCode): ?string {
        if (! $discountCode) {
            $cart->clearDiscount();
            return null;
        }
